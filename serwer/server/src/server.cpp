...
serwer::serwer(int serwer_port) {
  // deskryptor gniazda zwrocny przez, int socket(int domain, int type, int
  // protocol);
  ...
  if (setsockopt(mySocket, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) ==
      -1) // fukcja nadpisuje jeszcze nie zwolniony wczesniej uzywany port
  {
  ...
  serwerAddrress.sin_addr.s_addr =
      htonl(INADDR_ANY); // INADDR_ANY - automatycznie wybiera ip, zamiast
                         // inet_pton(AF_INET, ip, &(serwerAddrress.sin_addr));
  serwerAddrress.sin_port =
      htons(static_cast<uint16_t>(serwer_port)); // host to network byte order
  memset(&(serwerAddrress.sin_zero), '\0', 8);

  sin_size = sizeof(struct sockaddr_in);
}

serwer::~serwer() { std::cout << "Destructor end!" << std::endl; }
...
int serwer::receive() {
  ...
  arraySocket.erase(
      std::remove(arraySocket.begin(), arraySocket.end(), tempSocket),
      arraySocket.end());
  close(tempSocket);
